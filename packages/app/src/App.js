...
import { Router, getWrappers, getRoute } from './lib/router'
...
function Wrapper (props) {
  ...
  const { route } = router
  let routeInfo = getRoute(route)

  if (routeInfo.fullPage) return children
  else return <ArchipelAppWrapper {...props} />
}

function ArchipelAppWrapper (props) {
  const { children, router } = props
  const { route } = router
